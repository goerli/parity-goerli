...
fn make_spec(chain: &BlockChain, base_spec: Spec) -> Spec {
...
impl EthTester {
    pub fn from_chain(chain: &BlockChain) -> Self {
        return Self::from_chain_spec(chain, ethereum::new_frontier_test());
    }

    pub fn from_chain_spec(chain: &BlockChain, base_spec: Spec) -> Self {
        let tester = if ::ethjson::blockchain::Engine::NoProof == chain.engine {
            ...
            Self::from_spec_conf(make_spec(chain, base_spec), config)
        } else {
            Self::from_spec(make_spec(chain, base_spec))
        };
    ...
    pub fn from_spec_conf(spec: Spec, config: ClientConfig) -> Self {
        ...
        handler.extend_with(CliqueClient::new(&client).to_delegate());
